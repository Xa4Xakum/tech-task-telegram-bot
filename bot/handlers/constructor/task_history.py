...
async def send_corusel(msg: Message, state: FSMContext):
    ...
    if not user_answer:
        ...
        if task.deadline > datetime.now(): markup = kb.constructor.opened_tasks_corusel_with_answer
        else: markup = kb.constructor.opened_tasks_corusel
    else:
